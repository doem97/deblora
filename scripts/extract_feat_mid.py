...
def process_images_in_folder(folder_path, dift, category, img_size, time_steps, up_ft_index, ensemble_size, output_path, worker_idx, n_workers, mid_feat):
    """
...
def main(args):
    print(f"Runing >>>>>>>>>>>>> Worker {args.worker_idx} of {args.n_workers} workers extracting features from '{args.input_path}' to '{args.output_path}'")
    dift = SDFeaturizer4Eval(sd_id=args.sd_id, cat_list=["cargo", "fishing", "container", "tanker"], prompt_prefix=args.prompt_prefix)
...
if __name__ == "__main__":
    ...
    # add sample_step arg, which sets the time-step directly.
