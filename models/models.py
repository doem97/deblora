    ...
    def encode_text(self, text):
        try:
            ...
            text_features = torch.cat(
                [self.text_encoder(x) for x in text_split]
            )
...
class PeftModelFromCLIP(nn.Module):
    def __init__(self, cfg, clip_model, num_classes):
        ...
        if cfg.backbone.startswith("CLIP-ViT"):
            ...
            if cfg.fine_tuning:
                print("Fine-tuning: Turning off the tuner")
                self.tuner = None
            else:
            ...
            if cfg.fine_tuning:
                print("Fine-tuning: Don't initialize the tuner")
                self.tuner = None
            else:
        ...
        classifier_map = {
            ...
            "L2NormedClassifier": L2NormedClassifier,
            "LayerNormedClassifier": LayerNormedClassifier,
        }

        classifier_class = classifier_map.get(cfg.classifier)
        if classifier_class is None:
            raise ValueError(f"Invalid classifier: {cfg.classifier}")

        self.head = classifier_class(feat_dim, num_classes, dtype, **cfg)
...
class PeftModelFromViT(nn.Module):
    def __init__(self, cfg, vit_model, num_classes):
        ...
        if cfg.backbone.startswith("IN21K-ViT") or cfg.backbone.startswith(
            "SatMAE-ViT"
        ):
