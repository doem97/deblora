...
class MAEViTAdapter(nn.Module):
    """Adapter class to make MAE ViT compatible with standard ViT interface"""

    def __init__(self, mae_vit):
        super().__init__()
        self.patch_embed = mae_vit.patch_embed
        self.cls_token = mae_vit.cls_token
        self.pos_embed = mae_vit.pos_embed
        self.blocks = mae_vit.blocks
        self.norm = mae_vit.norm

        # Add properties to match standard ViT interface
        self.num_classes = 0  # Will be set by downstream tasks
        self.global_pool = "token"
        # Get embed_dim from the first block's dimensions
        self.num_features = self.blocks[0].norm1.normalized_shape[0]
        self.embed_dim = self.num_features  # Add this for compatibility
